...
public static class Loc
{
    ...
    private static readonly Dictionary<string, Dictionary<string, string>> Catalog = new()
    {
        [Fallback] = new()
        {
            ...
            ["tray.bedtime"] = "Bedtime in {0} min — start wrapping up.",
            ["tray.appblocked"] = "{0} is blocked and was closed.",
            ["tray.apptimeup"] = "Time's up for {0} today — it was closed.",
            ["tray.applimitwarn"] = "{0}: {1} min left before it closes.",
            ["tray.eyestrain.title"] = "Rest your eyes",
            ["tray.eyestrain.body"] = "Look at something about 20 feet away for 20 seconds.",
            ...
            ["settings.weekly.toggle"] = "Also cap total weekly time",
            ["settings.weekly.hours"] = "Weekly limit (hours)",
            ["settings.winddown"] = "Bedtime warning (minutes before)",
            ["settings.allowlist.title"] = "Apps that don't count",
            ["settings.blockapps.title"] = "Blocked apps",
            ["settings.blockapps.desc"] = "These apps are closed automatically whenever they are opened (one name per line).",
            ["settings.apptime.title"] = "Per-app daily limits",
            ["settings.apptime.desc"] = "Give an app its own minutes-per-day budget, separate from the overall limit. One \"app=minutes\" per line; the app is closed once its time is used up (resets at midnight).",
            ["settings.appweek.title"] = "Per-app weekly limits",
            ["settings.appweek.desc"] = "Give an app a minutes-per-week budget (Mon–Sun) on top of any daily limit. One \"app=minutes\" per line; the app is closed once the week's time is used up.",
            ["settings.blockvpnapps"] = "Block known VPN / Tor apps",
            ["settings.blockremoteapps"] = "Block remote-access apps (anti-scam)",
            ...
            ["settings.eyestrain.toggle"] = "Eye-rest reminders (20-20-20)",
            ["settings.eyestrain.interval"] = "Remind every (minutes)",
            ["settings.newapp.toggle"] = "Alert when a new app is used",
            ["settings.pause.title"] = "Breaks",
            ["settings.pause.desc"] = "Let the child take short self-service breaks that pause the timer, within limits you set.",
            ["settings.pause.toggle"] = "Allow breaks",
            ["settings.pause.budget"] = "Daily break budget (minutes)",
            ["settings.pause.max"] = "Longest single break (minutes)",
            ["settings.pause.cooldown"] = "Cooldown between breaks (minutes)",
            ["settings.pause.minactive"] = "Active time before first break (minutes)",
            ...
            ["settings.safesearch"] = "Force SafeSearch (Google/Bing/YouTube)",
            ["settings.privatebrowsing"] = "Disable private / incognito browsing",
            ["settings.strict.button"] = "Apply strict protection",
            ["settings.blocklist.toggle"] = "Subscribe to public blocklists",
            ["settings.blocklist.desc"] = "Automatically download and block large curated domain lists (updated every few hours). The list is capped so the PC stays fast.",
            ["settings.blocklist.adult"] = "Adult / pornography (StevenBlack)",
            ["settings.blocklist.ads"] = "Ads + malware + tracking (StevenBlack unified)",
            ["settings.blocklist.fakenews"] = "Fake news / misinformation (StevenBlack)",
            ["settings.blocklist.gambling"] = "Gambling / betting (StevenBlack)",
            ["settings.blocklist.social"] = "Social media — broad list (StevenBlack)",
            ["settings.blocklist.customurls"] = "Custom blocklist URLs (https, one per line)",
            ["settings.blocklist.allow"] = "Always-allow exceptions (one domain per line)",
            ["settings.categories.title"] = "Block by category",
            ["settings.categories.social"] = "Social media",
            ["settings.categories.gaming"] = "Gaming",
            ["settings.categories.streaming"] = "Streaming",
            ["settings.categories.adult"] = "Adult content",
            ["settings.categories.proxy"] = "VPN / proxy sites",
            ["settings.blocked.header"] = "Blocked sites (one per line)",
            ["settings.blocked.placeholder"] = "example.com\nroblox.com",
            ...
            ["event.BreakTaken"] = "Break taken",
            ["event.AppBlocked"] = "Blocked app closed",
            ["event.AppTimeLimitReached"] = "App time limit reached",
            ["event.AppFirstSeen"] = "New app used",
        ...
        ["de"] = new()
        {
            ...
            ["tray.bedtime"] = "Schlafenszeit in {0} Min — zum Ende kommen.",
            ["tray.appblocked"] = "{0} ist gesperrt und wurde geschlossen.",
            ["tray.apptimeup"] = "Zeit für {0} ist heute aufgebraucht — geschlossen.",
            ["tray.applimitwarn"] = "{0}: noch {1} Min, dann wird geschlossen.",
            ["tray.eyestrain.title"] = "Augen entspannen",
            ["tray.eyestrain.body"] = "Schau 20 Sekunden lang auf etwas etwa 6 Meter Entferntes.",
            ...
            ["settings.weekly.toggle"] = "Auch wöchentliche Gesamtzeit begrenzen",
            ["settings.weekly.hours"] = "Wochenlimit (Stunden)",
            ["settings.winddown"] = "Schlafenszeit-Warnung (Minuten vorher)",
            ["settings.allowlist.title"] = "Apps, die nicht zählen",
            ["settings.blockapps.title"] = "Gesperrte Apps",
            ["settings.blockapps.desc"] = "Diese Apps werden beim Öffnen automatisch geschlossen (ein Name pro Zeile).",
            ["settings.apptime.title"] = "Tägliche Limits pro App",
            ["settings.apptime.desc"] = "Gib einer App ein eigenes Minuten-pro-Tag-Budget, getrennt vom Gesamtlimit. Ein \"App=Minuten\" pro Zeile; die App wird geschlossen, sobald die Zeit aufgebraucht ist (Reset um Mitternacht).",
            ["settings.appweek.title"] = "Wöchentliche Limits pro App",
            ["settings.appweek.desc"] = "Gib einer App ein Minuten-pro-Woche-Budget (Mo–So) zusätzlich zu einem Tageslimit. Ein \"App=Minuten\" pro Zeile; die App wird geschlossen, sobald die Wochenzeit aufgebraucht ist.",
            ["settings.blockvpnapps"] = "Bekannte VPN-/Tor-Apps blockieren",
            ["settings.blockremoteapps"] = "Fernzugriff-Apps blockieren (Betrugsschutz)",
            ...
            ["settings.eyestrain.toggle"] = "Augenpausen-Erinnerung (20-20-20)",
            ["settings.eyestrain.interval"] = "Erinnern alle (Minuten)",
            ["settings.newapp.toggle"] = "Warnen, wenn eine neue App genutzt wird",
            ["settings.pause.title"] = "Pausen",
            ["settings.pause.desc"] = "Erlaube dem Kind kurze selbst gewählte Pausen, die den Timer anhalten — im von dir gesetzten Rahmen.",
            ["settings.pause.toggle"] = "Pausen erlauben",
            ["settings.pause.budget"] = "Tägliches Pausenkontingent (Minuten)",
            ["settings.pause.max"] = "Längste Einzelpause (Minuten)",
            ["settings.pause.cooldown"] = "Wartezeit zwischen Pausen (Minuten)",
            ["settings.pause.minactive"] = "Aktivzeit vor erster Pause (Minuten)",
            ...
            ["settings.safesearch"] = "SafeSearch erzwingen (Google/Bing/YouTube)",
            ["settings.privatebrowsing"] = "Privates / Inkognito-Surfen deaktivieren",
            ["settings.strict.button"] = "Strengen Schutz anwenden",
            ["settings.blocklist.toggle"] = "Öffentliche Sperrlisten abonnieren",
            ["settings.blocklist.desc"] = "Große kuratierte Domainlisten automatisch herunterladen und sperren (alle paar Stunden aktualisiert). Die Liste ist begrenzt, damit der PC schnell bleibt.",
            ["settings.blocklist.adult"] = "Erwachsene / Pornografie (StevenBlack)",
            ["settings.blocklist.ads"] = "Werbung + Malware + Tracking (StevenBlack unified)",
            ["settings.blocklist.fakenews"] = "Falschnachrichten / Desinformation (StevenBlack)",
            ["settings.blocklist.gambling"] = "Glücksspiel / Wetten (StevenBlack)",
            ["settings.blocklist.social"] = "Soziale Medien — breite Liste (StevenBlack)",
            ["settings.blocklist.customurls"] = "Eigene Sperrlisten-URLs (https, eine pro Zeile)",
            ["settings.blocklist.allow"] = "Ausnahmen – immer erlauben (eine Domain pro Zeile)",
            ["settings.categories.title"] = "Nach Kategorie sperren",
            ["settings.categories.social"] = "Soziale Medien",
            ["settings.categories.gaming"] = "Gaming",
            ["settings.categories.streaming"] = "Streaming",
            ["settings.categories.adult"] = "Erwachseneninhalte",
            ["settings.categories.proxy"] = "VPN-/Proxy-Seiten",
            ["settings.blocked.header"] = "Gesperrte Seiten (eine pro Zeile)",
            ["settings.blocked.placeholder"] = "example.com\nroblox.com",
            ...
            ["event.BreakTaken"] = "Pause genommen",
            ["event.AppBlocked"] = "Gesperrte App geschlossen",
            ["event.AppTimeLimitReached"] = "App-Zeitlimit erreicht",
            ["event.AppFirstSeen"] = "Neue App verwendet",
